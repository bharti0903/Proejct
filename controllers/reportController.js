...
const calculateProductivityScore = (entries) => {
  ...
  entries.forEach((entry) => {
    ...
    if (entry.category === "Study") score += hours * 8;
    else if (entry.category === "Social Media") score -= hours * 10;
    else if (entry.category === "Entertainment") score -= hours * 7;
    else if (entry.category === "Gaming") score -= hours * 9;
    else score -= hours * 2;
  });
...
const buildCategoryTotals = (entries) => {
  const totals = { "Social Media": 0, Entertainment: 0, Study: 0, Gaming: 0, Other: 0 };
  entries.forEach((entry) => {
    if (totals[entry.category] !== undefined) totals[entry.category] += entry.hours || 0;
    else totals.Other += entry.hours || 0;
  });
  return totals;
};

const buildTopWebsites = (entries, limit = 5) => {
  const map = {};
  entries.filter((entry) => entry.source === "extension").forEach((entry) => {
    ...
    map[key].visits += 1;
    if (entry.title) map[key].lastTitle = entry.title;
  });

  return Object.values(map)
    .sort((a, b) => b.totalHours - a.totalHours)
    .slice(0, limit)
    .map((site) => ({ ...site, totalHours: formatHours(site.totalHours) }));
};
...
const getWeeklyReportPage = async (req, res) => {
  try {
    ...
    for (const day of last7Days) {
      ...
      weeklyRows.push({
        label: day.toLocaleDateString("en-IN", { weekday: "long", day: "numeric", month: "short" }),
    ...
    const topCategory = CATEGORY_KEYS.sort((a, b) => weeklyCategoryTotals[b] - weeklyCategoryTotals[a])[0] || "No data";
    ...
    res.render("store/report", {
      ...
      monthlyTotal: 0,
      topCategory: "No data",
      weeklyCategoryTotals: { "Social Media": 0, Entertainment: 0, Study: 0, Gaming: 0, Other: 0 },
      monthlyCategoryTotals: { "Social Media": 0, Entertainment: 0, Study: 0, Gaming: 0, Other: 0 },
...
const exportWeeklyCsv = async (req, res) => {
  try {
    ...
    const header = ["Date", "Created At", "Category", "Hours", "Source", "Domain", "Title", "URL", "Notes"];
